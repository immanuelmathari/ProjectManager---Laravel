...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // $activeTasks = Task::query()->whereIn('status', ['pending', 'in_progress'])->limit(10)->get();
        // $activeTasksR = Task::query()->whereIn('status', ['pending', 'in_progress'])->where('assigned_user_id', $user->id)->limit(10)->get();
        $activeTasks = Task::query()->whereIn('status' , ['pending' , 'in_progress'])->where('assigned_user_id', $user->id)->limit(10)->get();
        // anything we get we must pass it throught its resource
        ...
        //     'activeTasks' => $activeTasksResource,
        //     'totalPendingTasks', 'myPendingTasks' , 'totalInProgressTasks' , 'myInProgressTasks' , 'totalCompletedTasks' , 'myCompletedTasks'
        // ]);
    }
}
